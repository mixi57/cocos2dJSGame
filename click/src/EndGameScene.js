...
var EndGameLayer = cc.Layer.extend({
	...
	ctor:function () {
		...
		var score = new cc.LabelTTF(String(g_Score+"分"),"黑体",80);
		score.setColor(cc.color(0, 0, 0));
		score.setPosition(winsize.width/2, winsize.height/3*2-50);
...
//		判断分数范围 
		var type = 3;
		if(g_Score <= g_Config.ScoreScope[0][1]){
			type = 3;
		}else if (g_Config.ScoreScope[1][0] < g_Score && g_Score <= g_Config.ScoreScope[1][1]) {
			type = 2;
		}else if (g_Config.ScoreScope[2][0] < g_Score && g_Score <= g_Config.ScoreScope[2][1]) {
			type = 1;
		}else if (g_Config.ScoreScope[3][0] < g_Score && g_Score < g_Config.ScoreScope[3][1]) {
			type = 0;
		}
	...
	setLevelType : function(type){
		var levelogo = new cc.Sprite(g_Config.LevelLogo[type]);
		levelogo.setPosition(cc.winSize.width/2, cc.winSize.height/4*3);
		this.addChild(levelogo);
	}
}
);
